...
function invalidStates() {
  ...
  const idle: TurnState = { status: "idle", id: turn.id, agent: turn.agent, steps: turn.steps, child: ref("completion", "request") };
  // @ts-expect-error preparation requires a validated positive allowance
  const preparing: TurnState = { status: "preparing_model", turn, child: ref("prepare", "request") };
  ...
  const executing: TurnState = { status: "executing_tools", turn, child: ref("completion", "request") };
