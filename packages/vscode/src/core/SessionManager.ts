...
export type AgentConnectionError =
  { kind: "auth-cancelled" } | { kind: "connect-failed"; message: string };
...
export class SessionManager extends EventEmitter {
  ...
  private pendingUsage = new Map<string, UsageUpdate>();
  ...
  private async getWorkspaceCwd(): Promise<string> {
    const folders = vscode.workspace.workspaceFolders ?? [];
    const active = vscode.window.activeTextEditor?.document.uri;
    let selected = workspaceCwd(
      vscode.workspace.getConfiguration("acp").get<string>("defaultWorkingDirectory", ""),
      folders.map((folder) => folder.uri.fsPath),
      active ? vscode.workspace.getWorkspaceFolder(active)?.uri.fsPath : undefined,
    );
    if (!selected) {
      const folder = await vscode.window.showWorkspaceFolderPick({
        placeHolder: "Select the workspace for this agent session",
      });
      if (!folder) throw new Error("Agent connection cancelled: no workspace was selected");
      selected = { cwd: folder.uri.fsPath, source: "workspace_picker" };
    }
    log("Workspace selected for agent process and session", selected);
    return selected.cwd;
  }

  private summarizeCapabilities(
    caps: AgentCapabilities | undefined | null,
  ...
  async connectToAgent(agentName: string): Promise<SessionInfo> {
    ...
    try {
      const workspaceCwd = await this.getWorkspaceCwd();
  ...
  private async createAcpSession(
    ...
    const sessionInfo: SessionInfo = {
      ...
      agentName,
      agentDisplayName:
        connInfo.initResponse.agentInfo?.title ||
  ...
  private drainPending(sessionInfo: SessionInfo): void {
    const usage = this.pendingUsage.get(sessionInfo.sessionId);
    if (usage) {
      sessionInfo.usage = usage;
      this.pendingUsage.delete(sessionInfo.sessionId);
    }
  ...
  async setModel(sessionId: string, modelId: string): Promise<void> {
    ...
    throw new Error("This agent has not advertised a model configuration option");
  }
   ...
   */
  applyUsageUpdate(sessionId: string, usage: UsageUpdate): void {
    const session = this.sessions.get(sessionId);
    if (session) session.usage = usage;
    else this.pendingUsage.set(sessionId, usage);
  }
      ...
      if (existing) {
        ...
        if (conn) {
          this.capabilities.set(
            agentName,
            this.summarizeCapabilities(conn.initResponse.agentCapabilities),
          );
          return conn;
        }
      }
  ...
  async listSessions(
    ...
    if (this.historyStore && !opts.cursor) {
      this.historyStore.reconcileFromAgent(agentName, new Set(sessions.map((s) => s.sessionId)));
    }
  ...
  async loadSession(agentName: string, sessionId: string): Promise<SessionInfo> {
    ...
    const cwd = await this.getWorkspaceCwd();
    ...
    const placeholder: SessionInfo = {
      ...
      agentName,
      agentDisplayName:
        conn.initResponse.agentInfo?.title || conn.initResponse.agentInfo?.name || agentName,
      cwd,
      createdAt: new Date().toISOString(),
