...
type State = { status: "idle" } | { status: "running"; value: number } | { status: "failed"; error: string };
type Event = { type: "start" } | { type: "add" } | { type: "bad" } | { type: "failed"; error: string };
...
const decide = defineMachine<State, Event, Command>({
  ...
  running: {
    ...
    bad: () => { throw new Error("invalid decision"); },
...
test("commits a complete frozen state before dispatching commands or reentrant events", async () => {
  const seen: State[] = [];
  const actor = new Actor<State, Event, Command>({ status: "idle" }, decide, () => {
...
test("a rejected decision preserves state and dispatches nothing; the mailbox recovers", async () => {
  let commands = 0;
  const actor = new Actor<State, Event, Command>({ status: "idle" }, decide, () => { commands++; return undefined; },
    (_, error) => ({ type: "failed", error: String(error) }));
  await actor.send({ type: "start" });
