...
export class ChatWebviewProvider implements vscode.WebviewViewProvider {
  ...
  private handleSessionUpdate(update: SessionNotification): void {
    ...
    if (update.update.sessionUpdate === "usage_update")
      this.sessionManager.applyUsageUpdate(update.sessionId, update.update);
...
<head>
  ...
  <style>
    ...
    .acp-diff-before { background: var(--vscode-diffEditor-removedTextBackground); }
    .acp-diff-after { background: var(--vscode-diffEditor-insertedTextBackground); }
...
<body>
  ...
  <div class="input-area" id="inputArea">
    ...
    <div class="input-toolbar">
      ...
      <div id="sessionUsage" role="status" aria-live="polite"></div>
  ...
  <script nonce="${nonce}">
    ...
    function showSessionConnected(session) {
      if (sessionState?.sessionId !== session.sessionId) {
        const indicator = document.getElementById('sessionUsage');
      ...
      saveState();
      showSessionConnectedFromState(sessionState);
      if (session.usage) handleUpdate(session.usage);
    ...
    function showSessionConnectedFromState(ss) {
      hasActiveSession = true;
      hideEmpty();
      if (ss.usage) handleUpdate(ss.usage);
    ...
    function showToolContent(toolCallId, content) {
      ...
      if (!card) return;
      let output = card.querySelector('.acp-tool-content');
      if (!output) {
    ...
    function handleUpdate(update) {
      ...
      switch (type) {
        ...
        case 'usage_update': {
          if (sessionState) { sessionState.usage = update; saveState(); }
          const indicator = document.getElementById('sessionUsage');
          if (indicator) indicator.textContent = 'Context: ' + update.used + ' / ' + update.size +
            (update.cost ? ' · Session cost: ' + update.cost.amount + ' ' + update.cost.currency : '');
          break;
        }
