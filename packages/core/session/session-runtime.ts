...
import { configureSession, type ConfiguredSession } from "./runtime/configure.ts";
import { planAdoption, registryDifferences, type Adoption } from "./runtime/registry-adoption.ts";
import {
  decideSession,
  type CommandReceipt,
  type SessionCommand,
...
import { SeedSchema, SystemVersionSchema, type Seed, type SessionInput } from "./types.ts";
...
async function initialize(configured: ConfiguredSession, seed: Seed): Promise<SessionRuntime> {
  const built = build(configured, seedConversation(seed, configured.resolvers));
...
function build(configured: ConfiguredSession, initial: JournalState, restoring = false) {
  const {
    agents,
    tools,
    resolvers,
  ...
  async function initializeBranch(seed: Seed) {
    ...
    try {
      ...
      return await initialize(configured, seed);
    } finally {
