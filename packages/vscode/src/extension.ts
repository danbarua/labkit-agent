...
export function activate(context: vscode.ExtensionContext): void {
  const environments = [
    process.env,
    ...Object.values(getAgentConfigs()).map((config) => config.env ?? {}),
  ];
  ...
  configureDiagnostics(join(context.globalStorageUri.fsPath, "logs"), secrets);
  ...
  // --- UI ---
  const workspaceCwd = () =>
    sessionManager.getActiveSession()?.cwd ?? vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
  ...
  const forgetSessionCmd = vscode.commands.registerCommand(
    "acp.forgetSession",
    async (arg?: any) => {
  ...
  sendEvent("extension/activated", {
    version:
      vscode.extensions.getExtension("labkit.labkit-acp-client")?.packageJSON?.version ?? "unknown",
  });
