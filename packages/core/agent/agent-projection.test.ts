...
const project = (messages: readonly AgentMessage[], outcome: Outcome) =>
  projectConversationPrompt({
    log: [{ agent: AgentIdSchema.parse("writer"), messages, outcome }],
    turn: turnData(),
    agent,
  });

test("missing results in completed history or current turns raise errors", () => {
  expect(() => project(exchange(), { kind: "completed" })).toThrow(
    "logged turn 1: missing results for b",
  );
  expect(() =>
    projectConversationPrompt({ log: [], turn: { ...turnData(), messages: exchange() }, agent }),
...
test("interrupted tails retain matched results without altering source records", () => {
  for (const outcome of [
    { kind: "aborted" },
    { kind: "exhausted" },
    { kind: "failed", error: { message: "offline" } },
  ] as const) {
    const messages = exchange();
    const projected = project(messages, outcome);
    const assistant = projected[0]!;
    expect(assistant.role === "assistant" && assistant.tool_calls?.map((call) => call.id)).toEqual([
      "a",
    ]);
...
test("interruption does not excuse missing results in earlier exchanges", () => {
  expect(() =>
    project([...exchange(), { role: "assistant", text: "finished" }], { kind: "aborted" }),
  ).toThrow("missing results for b");
});

test("orphan, unmatched and duplicate results raise projection errors", () => {
  expect(() =>
    project(MessagesSchema.parse([{ role: "tool", callId: "a", text: "orphan" }]), {
      kind: "aborted",
    }),
  ).toThrow("orphan result a");
  for (const callId of ["a", "unknown"]) {
    expect(() =>
      project(MessagesSchema.parse([...exchange(), { role: "tool", callId, text: "bad" }]), {
        kind: "aborted",
      }),
