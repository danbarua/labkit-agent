...
| Usage and cost                  | Validated response accounting survives the completion port, journal receipt and restore through `lastCompletionUsage`.           | ACP `usage_update` is missing: real context capacity and cumulative session cost require a public source. Do not fabricate either from arbitrary deltas.                         |
