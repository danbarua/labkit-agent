...
function harness(overrides: Partial<RuntimeOptions> = {}) {
  ...
  const runtime = createAgentRuntime({
    agent: "writer",
    agents: new Map([
      ["writer", { model: "write", tools: ["lookup"] }],
...
test("fork clones a completed session under fresh identities and continues independently", async () => {
  ...
  expect(h.requests[2]!.request.messages.map((message) => message.content)).toEqual([
    "first",
    "answer",
    "fork branch",
  ]);
...
test("fork waits for Done, includes the settled answer, and preserves barge-in", async () => {
  ...
  let published = false;
  const pending = h.runtime.fork().then((fork) => {
    published = true;
    return fork;
  });
  ...
  expect(fork.snapshot.conversation.log[0]?.messages.map((message) => message.text)).toEqual([
    "first",
    "replacement",
    "current",
  ]);
...
test("fork and compaction wait through the complete tool/model loop without replaying tools", async () => {
  ...
  let forks = 0;
  const normal = h.runtime.fork().then((value) => {
    forks++;
    return value;
  });
  const compacted = h.runtime
    .compact([{ role: "user", text: "Condensed context" }])
    .then((value) => {
      forks++;
      return value;
    });
  ...
  tools[0]!.resolve("one");
  await until(
    () => h.runtime.snapshot.children.filter((child) => child.ref.kind === "tool").length === 1,
  );
  expect(forks).toBe(0);
  tools[1]!.resolve("two");
  await until(() => h.requests.length === 2);
  ...
  expect(compact.snapshot.conversation.context).toMatchObject([
    { role: "user", text: "Condensed context" },
  ]);
  expect(compact.snapshot.conversation.origin).toMatchObject({
    kind: "compaction",
    parent: sourceId,
  });
...
test("compaction replaces earlier context and history without mutating any ancestor", async () => {
  ...
  await until(() => h.requests.length === 2);
  expect(h.requests[1]!.request.messages.map((message) => message.content)).toEqual([
    "new summary",
    "next",
  ]);
...
test("invalid compaction is rejected atomically without changing source state or starting I/O", async () => {
  ...
  for (const context of [
    null,
    [{ role: "user", text: 42 }],
    [{ role: "tool", callId: "a", text: "orphan" }],
...
test("handoff and exhausted turns publish forks with the final agent and fresh allowance", async () => {
  ...
  expect(fork.snapshot.conversation.turn).toMatchObject({
    status: "idle",
    agent: "reviewer",
    steps: 1,
  });
...
test("forks inherit validated configuration rather than mutated caller registries", async () => {
  ...
  const tools = new Map([
    ["lookup", defineTool({ input: z.object({}), run: () => "original result" })],
  ]);
